...
def main(args):
    ...
    shapenet_obj_dir = osp.join(path_util.get_rndf_obj_descriptions(), obj_class + '_centered_obj_normalized')

    save_dir = osp.join(path_util.get_rndf_data(), 'demos', obj_class, args.exp)
    util.safe_makedirs(save_dir)

    # use the training split we saved from data generation
    train_objects = np.loadtxt(osp.join(path_util.get_rndf_share(), '%s_train_object_split.txt' % obj_class), dtype=str).tolist()
