...
class VisionModule:
    ...
    def initialize_cameras(self, camera_sys):
    ...
    def pybullet_seg(self, obj_ids):
        assert self.seg_method == 'pb_seg', 'Seg method does not use PyBullet'
        return self.camera_sys.get_pb_seg(obj_ids)

    def language_seg(self, captions, centroid_thresh=0.1, detect_thresh=0.15):
        pcd_2ds, rgb_imgs, valid_depths = self.camera_sys.get_all_real_views()
 
        label_to_pcds = {}
        label_to_scores = {}

        for i, rgb, pcd_2d, valid in zip(range(len(rgb)), rgb_imgs, pcd_2ds, valid_depths):

            all_obj_masks = {}
            all_obj_bb_scores = {}       
            is_bbox = True if self.seg_method in {'bbox', 'owl'}  else False
            ...
            for obj_label, obj_masks in all_obj_masks.items():
                ...
                obj_pcds, obj_scores = pcds_from_masks(pcd_2d, valid, obj_masks, all_obj_bb_scores[obj_label], is_bbox=is_bbox)
